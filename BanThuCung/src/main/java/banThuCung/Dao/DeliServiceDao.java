...
public class DeliServiceDao extends BaseDao {
	public String generateId() {
		String prefix = "DS";
		String count = _jdbcTemplate.queryForObject("select count(deli_ser_id) from delivery_service", Integer.class).toString();
	...
	public int saveDeliService(DeliService deliService) {
		String sql = "Insert into delivery_service(deli_ser_id, deli_com_id, type, price, time)"
				+ "values(?,?,?,?,?)";
		return _jdbcTemplate.update(sql, generateId(), deliService.getDeli_com_id(), deliService.getType(),
