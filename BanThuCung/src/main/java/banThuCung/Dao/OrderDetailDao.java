...
public class OrderDetailDao extends BaseDao {
	public String generateId() {
		String prefix = "OR";
		String count = _jdbcTemplate.queryForObject("select count(order_id) from order_detail", Integer.class).toString();
	...
	public int saveOrderDetailAutoID(Order_detail order_detail) {
		String sql = "Insert into order_detail(order_id, pro_id, proper_id, quantity, total, paid) "
				+ "values(?,?,?,?,?,?)";
		return _jdbcTemplate.update(sql,generateId(), order_detail.getPro_id(), order_detail.getProper_id(),
				order_detail.getQuantity(),order_detail.getTotal(), order_detail.isPaid());
	}
